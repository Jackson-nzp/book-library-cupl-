...
def reserve(token,cookies,ide,seat,tor_date):
    ...
    res_param = {
            ...
            "memberKind":1,
            "resvMember":[ide],
            "resvBeginTime":str(tor_date)+" 16:10:00",
...
def main():
#从文件中获取身份信息
    person=pd.read_csv('D:\\01 学习资料\\00 研究\\04 计算机\\01 工作储备\\02 语言\\02 python\\03 爬虫\\00 项目\\01 图书馆抢座位\\01 success\\01\identity.csv')
    seats=pd.read_csv('D:\\01 学习资料\\00 研究\\04 计算机\\01 工作储备\\02 语言\\02 python\\03 爬虫\\00 项目\\01 图书馆抢座位\\01 success\\01\seat.csv')

    cur_time = time.strftime("%H:%M:%S", time.localtime())
    print(cur_time)
    num=person.iloc[0][0]
    pwd=person.iloc[0][1]
    domain_cookie=person.iloc[0][3]
    #如果要抢座位，做好还是能同时操作多人，如何实现多线程
    token,cookies=login(int(num),int(pwd),domain_cookie)
    print(token,cookies)
    flag = True
    while cur_time < '21:40:00':
        if flag is True:
            print('当前时段暂不可预约')
            flag = False
        cur_time = time.strftime("%H:%M:%S", time.localtime())
    ...
    #获取座位信息，之所以不选择迭代是因为座位有区分，靠近窗口等，故不做尝试。而且座位号并不连续
    ide=person.iloc[0][2]
    for i in range(seats.shape[0]):
        seat=seats.iloc[i][1]
        tmp=reserve(token,cookies,int(ide),int(seat),tor_date)
